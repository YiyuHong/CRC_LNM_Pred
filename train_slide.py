...
args.train_lib = r'/Arontier/People/hongyiyu/Data/Samsung_Colon2/Tile_Dicts_200828/TS512_OL0_Fold2_CV5/CV_1_Train.pth'
args.val_lib = r'/Arontier/People/hongyiyu/Data/Samsung_Colon2/Tile_Dicts_200828/TS512_OL0_Fold2_CV5/CV_1_Val.pth'

args.num_class = 2
args.tile_size = 256
args.tile_batch_size = 100000


args.wsi_batch_size = 1
args.wsi_tile_max = 1000 ## res34-256 -> 2000, res34-512 -> 500
...
args.lr = 0.001  # 0.001
args.dropout = 0.25
args.output = r'/Arontier/People/hongyiyu/Data/Samsung_Colon2/Tile_Dicts_200828/TS512_OL0_Fold2_CV5/Results/R18_P256_B128_Fold1/R18_FLB_P256_AO_DO25_B1_M1000_W_E11'
if not os.path.exists(args.output):
    os.makedirs(args.output)

args.save_model_every = 1
args.model_path = r''
# args.model_path = r'/Arontier/People/hongyiyu/Data/Samsung_Colon2/Tile_Dicts/TS2048_OL0/Results/Res34_Cli_P256_AttenOri_Batch4_Balanced_Fold3/checkpoint_2.pth'

args.ft_model_path = r'/Arontier/People/hongyiyu/Data/Samsung_Colon2/Tile_Dicts_200828/TS512_OL0_Fold2_CV5/Results/R18_P256_B128_Fold1/checkpoint_11.pth'
...
def main():
    ...
    optimizer_model = RAdam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)  # PlainRAdam
    ...
    if args.model_path != '':
        ...
        model.to(args.device)
        optimizer_model.load_state_dict(model_checkpoint['optimizer'])
        resume_epoch = model_checkpoint['epoch']

    normalize = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
    attention_train_trans = transforms.Compose(
        [Image_Augmentation_Alb(args.tile_size), transforms.ToTensor(), normalize])
    attention_val_trans = transforms.Compose([transforms.Resize(args.tile_size), transforms.ToTensor(), normalize])

    train_dset = attention_data(args.train_lib, attention_transform=attention_train_trans)
    val_dset = attention_data(args.val_lib, attention_transform=attention_val_trans)

    attention_train_loader = torch.utils.data.DataLoader(
        train_dset,
...
#         criterion = nn.CrossEntropyLoss().to(args.device)
    ...
    fconv.write('epoch, train_loss, train_auc, val_loss, val_auc, tnr_tpr100\n')
    fconv.close()

    for epoch in range(resume_epoch, args.nepochs):
        the_epoch_train_targets, the_epoch_train_slide_probs, train_loss = train_single(epoch, model, train_dset,
                                                                                        attention_train_loader,
                                                                                        optimizer_model, criterion)
        # scheduler_optimizer_score.step()

        the_epoch_val_targets, the_epoch_val_slide_probs, val_loss = val_single(epoch, model, val_dset,
                                                                                attention_val_loader, criterion)

        train_results = cal_slide_metrics(the_epoch_train_targets, the_epoch_train_slide_probs)
        ...
        val_slide_probs_df.to_csv(os.path.join(args.output, f"slide_probs_{epoch + 1}.csv"), index=False)

        print(f"Train\tEpoch: [{epoch + 1}/{args.nepochs}]\tTrain_Loss: {train_loss}\t Train_AUC: {train_results['auc']}")
        print(f"Val\tEpoch: [{epoch + 1}/{args.nepochs}]\tVal_Loss: {val_loss}\t Val_AUC: {val_results['auc']}")

        fconv = open(os.path.join(args.output, 'result_log.csv'), 'a')
        fconv.write(
            '{}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}\n'.format(epoch + 1, train_loss, train_results['auc'], val_loss,
                                                                  val_results['auc'], val_results['tnr_tpr100']))
        fconv.close()
...
def train_single(epoch, model, train_dset, attention_train_loader, optimizer_model, criterion):
    model.train()
    train_dset.setmode(3)

    if args.balance_flag:
        slide_idxs_batch_sampled = train_dset.make_train_slide_data_balanced(args.wsi_batch_size)
    else:
        slide_num = len(train_dset.slidenames)
        slide_idxs = [i for i in range(slide_num)]
        random.shuffle(slide_idxs)
        slide_idxs_batch_sampled_total = [slide_idxs[i:i + args.wsi_batch_size] for i in
                                          range(0, len(slide_idxs), args.wsi_batch_size)]
        slide_idxs_batch_sampled = slide_idxs_batch_sampled_total[:-1]  ## batch norm

    running_loss = 0.
    the_epoch_train_targets = []
    the_epoch_train_slide_probs = []

    for slide_batch_i, slide_idxs_batch in enumerate(slide_idxs_batch_sampled):
        batch_slide_targets = []
        batch_slide_clinical_features = []
        for slide_i, the_slide_idx in enumerate(slide_idxs_batch):
            # print(f"Train Slide {slide_i}/{len(slide_idxs)}")
            the_slide_target = torch.from_numpy(np.array(train_dset.targets[the_slide_idx])).to(args.device,
                                                                                                dtype=torch.long)
            batch_slide_targets.append(the_slide_target)

            the_slide_clinical_features = torch.from_numpy(np.array(train_dset.clinical_features[the_slide_idx])).to(
                args.device, dtype=torch.float)
            batch_slide_clinical_features.append(the_slide_clinical_features)

            the_epoch_train_targets.append(train_dset.targets[the_slide_idx])
        ...
        slide_batch_patch_idxs = train_dset.make_the_slide_tile_data_train(slide_idxs_batch)
        ...
        print(
            f'Train\tEpoch: [{epoch + 1}/{args.nepochs}]\tSlide Batch: [{slide_batch_i + 1}/{len(slide_idxs_batch_sampled)}]\tLoss: {loss.item()}')
        batch_slide_output_softmax = F.softmax(batch_slide_output)
        the_epoch_train_slide_probs.extend(batch_slide_output_softmax.detach()[:, 1].cpu().tolist())

    the_epoch_train_targets = np.array(the_epoch_train_targets)
    the_epoch_train_slide_probs = np.array(the_epoch_train_slide_probs).squeeze()

    return the_epoch_train_targets, the_epoch_train_slide_probs, running_loss / (slide_batch_i + 1)
...
def val_single(epoch, model, val_dset, attention_train_loader, criterion):
    ...
    for slide_batch_i, slide_idxs_batch in enumerate(slide_idxs_batch_sampled):
        ...
        batch_slide_targets.append(the_slide_target)
        the_slide_clinical_features = torch.from_numpy(np.array(val_dset.clinical_features[slide_idxs])).to(args.device, dtype=torch.float)
        batch_slide_clinical_features.append(the_slide_clinical_features)
        ...
        print(f'Val\tEpoch: [{epoch + 1}/{args.nepochs}]\tSlide Batch: [{slide_batch_i + 1}/{len(slide_idxs_batch_sampled)}]\tLoss: {np.mean(sampled_loss)}')
    ...
    return the_epoch_val_targets, the_epoch_val_slide_probs, running_loss / (slide_batch_i + 1)
...
class res_atten_net(nn.Module):
    def __init__(self, ft_dim=512, num_class=2, ft_model_path='', freeze_mode=-1, clinical_feature_flag=False, dropout=0):
    ...
    def make_the_slide_tile_data_train(self, selected_slide_idxs):
        ...
        for selected_slide_idx in selected_slide_idxs:
            ...
            if args.wsi_tile_max > selected_tile_num:
                the_batch_slide_grid_idxs.extend(np.random.choice(the_slide_grid_idxs, selected_tile_num))
                slide_batch_patch_idxs.append(slide_batch_patch_idxs[-1] + selected_tile_num)

            else:
                the_batch_slide_grid_idxs.extend(np.random.choice(the_slide_grid_idxs, args.wsi_tile_max))
